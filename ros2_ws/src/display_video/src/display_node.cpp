...
public:
  ...
  {
    // Đặt QoS là BEST_EFFORT để nhận video mượt nhất (giống kiểu UDP)
    // Nếu sender dùng RELIABLE thì cái này vẫn nhận được.
    rclcpp::QoS qos_profile(10);
    qos_profile.reliability(rclcpp::ReliabilityPolicy::BestEffort);

    // Tạo Subscriber lắng nghe topic "image_raw"
...
private:
  void topic_callback(const sensor_msgs::msg::Image::SharedPtr msg)
  {
    ...
    try {
      // TRƯỜNG HỢP 1: Nếu encoding là "nv12" (Xử lý thủ công)
      if (msg->encoding == "nv12") {
        // NV12 là định dạng YUV 4:2:0.
        // Cấu trúc bộ nhớ: Y plane (full size) + UV plane (half size interleaved).
        // Tổng chiều cao trong bộ nhớ sẽ là height * 1.5
        
        // 1. Tạo Mat bao bọc lấy dữ liệu raw (Không copy data, chỉ trỏ pointer)
        // Lưu ý: Chiều cao phải là height * 3 / 2
        cv::Mat nv12_mat(msg->height * 3 / 2, msg->width, CV_8UC1, const_cast<uint8_t*>(msg->data.data()));

        // 2. Chuyển đổi NV12 sang BGR để hiển thị
        cv::cvtColor(nv12_mat, display_frame, cv::COLOR_YUV2BGR_NV12);
      } 
      // TRƯỜNG HỢP 2: Các định dạng khác (dùng cv_bridge như bình thường)
      else {
