...
#include "sensor_sc230ai_config.h"

MIPI_SENSOR_INFO_S SENSOR_2LANE_SC230AI_30FPS_10BIT_LINEAR_INFO = {
	...
	.sensorInfo = {
		...
		.sensor_addr = 0x30, // sensor i2c 设备地址
		...
		.sensor_name = "sc230ai", // sensor的名字，在libcam.so中会根据这个名字组合出 libf37.so
...
MIPI_ATTR_S MIPI_2LANE_SENSOR_SC230AI_30FPS_10BIT_LINEAR_ATTR = {
    .mipi_host_cfg = {
        ...
        .mipiclk = 1200, // sensor 输出总的 mipibit rate, 单位Mbits/秒
        .fps = 30, // sensor 输出实际帧率, 和 MIPI_SENSOR_INFO_S 是什么关系？
        .width = 1920, // sensor实际输出的分辨率，一般根据sensor供应商的配置参数配置就行
        .height = 1080,
        .linelenth = 2800, // sensor 输出带 blanking 的总行长， 一般根据sensor供应商的配置参数配置就行
        .framelenth = 1200, // sensor 输出带 blanking的总行数
...
VIN_DEV_ATTR_S DEV_ATTR_SC230AI_LINEAR_BASE = {
    .stSize = { // 输入的数据
...
VIN_PIPE_ATTR_S PIPE_ATTR_SC230AI_LINEAR_BASE = {
	...
	.calib = { // 是否开启 sensor 矫正数据加载，1 是开启，0 是关闭。ISP使用这些数据
		...
		.lname = "libsc230ai_linear.so", // 对应使用的校准库
	}
};

VIN_DIS_ATTR_S DIS_ATTR_SC230AI_LINEAR_BASE = {
	.picSize = { // 输入数据宽高
...
VIN_LDC_ATTR_S LDC_ATTR_SC230AI_LINEAR_BASE = {
