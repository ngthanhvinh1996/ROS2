...
/** 1944P **/
MIPI_SENSOR_INFO_S SENSOR_2LANE_OV5647_15FPS_1944P_10BIT_LINEAR_INFO =
{
	.deseEnable = 0, // 该 sensor 是否有 serdes（串解器）
	.inputMode = INPUT_MODE_MIPI, // sensor 接入方式,mipi还是dvp
	.sensorInfo = {
		...
		.fps = 15, // 帧率，用来选择使用哪一组帧率的sensor参数
		.resolution = 1944, // sensor 行宽, 必须要和mipi属性配置一致
		.sensor_addr = 0x36, // sensor i2c 设备地址
		.entry_index = 0, // sensor 使用的 mipi 索引, 0~3，对应mipi host的序号
		.sensor_mode = NORMAL_M, // sensor 工作模式， 1 是 normal,2 是dol2,3 是 dol3
		.reg_width = 16, // sensor 寄存器地址宽度
...
MIPI_ATTR_S MIPI_2LANE_SENSOR_OV5647_15FPS_1944P_10BIT_LINEAR_ATTR =
{
    .mipi_host_cfg = {
        .lane = 2, // 硬件上sensor用了几个mipi数据lane，f37用的1 lane, os8a10 4K的用了4 lane
        .datatype = 0x2b, // sensor的输出数据类型，请参考 《X3J3平台AIOT媒体系统接口手册.pdf》第3.5.36节 DATA TYPE
        .mclk = 24, // mipi 模块主时钟，目前默认是24MHz
        .mipiclk = 800, // sensor 输出总的 mipibit rate, 单位Mbits/秒
        .fps = 15, // sensor 输出实际帧率, 和 MIPI_SENSOR_INFO_S 是什么关系？
        .width = 2592, // sensor实际输出的分辨率，一般根据sensor供应商的配置参数配置就行
        .height = 1944,
        .linelenth = 2844, // sensor 输出带 blanking 的总行长， 一般根据sensor供应商的配置参数配置就行
        .framelenth = 1968, // sensor 输出带 blanking的总行数
        .settle = 30, // sensor 输出实际Ttx-zero + Ttx-prepare时间（ clk 为单位）, 一般0-100之间尝试
		.channel_num = 1, // 使用虚通道的个数, linear 用一个，dol2 用 2 个， dol3 用3个
		.channel_sel = {0} // 保存每个虚通道的值，在调用HB_VIN_SetDevVCNumber接口时用第一个，调用HB_VIN_AddDevVCNumber用后面的2 3 4的配置
    },
...
VIN_DEV_ATTR_S DEV_ATTR_OV5647_15FPS_1944P_LINEAR_BASE = {
    .stSize = { // 输入的数据
		.format = 0, // 像素格式， format 为 0 代表是raw, 根据 pixel_lenght 来表示究竟是 raw8, raw12 还是 raw16 。
		.width = 2592, // 数据宽
		.height = 1944, // 数据高
    ...
    .outDdrAttr = { // sif(dev) 的输出到 ddr 配置
        .stride = 3248, // 硬件 stride 跟格式匹配，通过行像素根据raw数据bit位数计算得来（而且结果要32对齐），8bit：x1, 10bit: x1.25 12bit: x1.5,例F37 raw10，1920 x 1.25 = 2400
        .buffer_num = 8, // dev 输出到 ddr 的 buf 个数，最少可设置为6
        //.frameDepth = 1, // 默认不配置最多 get 的帧数 , buffer_num 是总 buff数量，建议 frameDepth 值最大是ddrOutBufNum – 4
    },
    .outIspAttr = { // sif 到 isp 一些属性设置
...
VIN_PIPE_ATTR_S PIPE_ATTR_OV5647_15FPS_1944P_LINEAR_BASE = {
	.ddrOutBufNum = 6, // ISP->GDC ddr的buf数量，最少可以设置成 2
	.snsMode = SENSOR_NORMAL_MODE, // sensor 工作模式，要和MIPI_SENSOR_INFO_S配置的sensor_mode一致
	.stSize = { // 数据格式同 VIN_DEV_SIZE_S
		.format = 0,
...
VIN_DIS_ATTR_S DIS_ATTR_OV5647_15FPS_1944P_LINEAR_BASE = {
	.picSize = { // 输入数据宽高
		.pic_w = 2591, // 需要设置比接入尺寸 -1 的 size, 如果 ISP 输出1920 , 则这里设置 1919
		.pic_h = 1943, // 需要设置比接入尺寸 -1 的 size
	},
	.disPath = { // 输出类型
		.rg_dis_enable = 0, // 是否开启
		.rg_dis_path_sel = 1, // 决定进dis的数据来源，0代表ISP输出给dis，1代表LDC输出给dis
...
VIN_LDC_ATTR_S LDC_ATTR_OV5647_15FPS_1944P_LINEAR_BASE = {
  .ldcEnable = 0, // LDC 是否使能
  .ldcPath = { // 输出类型
	 .rg_y_only = 0, // ???
	 .rg_uv_mode = 0, // ???
  ...
  .cStartAddr = 786432, // Iram 使用地址的内存地址
  .picSize = { // 输入的尺寸
	 .pic_w = 2591, // 需要设置比接入尺寸 -1 的 size, 如果 ISP 输出 1920 ,则这里设置 1919
	 .pic_h = 1943,
  },
  .lineBuf = 99, // 值设置 99, 为什么？
  .xParam = { // 参数需要 tuning
...
/** 480P **/
MIPI_SENSOR_INFO_S SENSOR_2LANE_OV5647_60FPS_480P_10BIT_LINEAR_INFO =
{
	.deseEnable = 0, // 该 sensor 是否有 serdes（串解器）
	.inputMode = INPUT_MODE_MIPI, // sensor 接入方式,mipi还是dvp
	.sensorInfo = {
		...
		.bus_type = 0, // 访问总线类型， 0 是 i2c,1 是 spi
		.bus_num = 1, // 总线号，根据具体板子硬件原理图确定 , 不配置默认 i2c5
		.fps = 60, // 帧率，用来选择使用哪一组帧率的sensor参数
		.resolution = 480, // sensor 行宽, 必须要和mipi属性配置一致
		.sensor_addr = 0x36, // sensor i2c 设备地址
		.entry_index = 0, // sensor 使用的 mipi 索引, 0~3，对应mipi host的序号
		.sensor_mode = NORMAL_M, // sensor 工作模式， 1 是 normal,2 是dol2,3 是 dol3
		.reg_width = 16, // sensor 寄存器地址宽度
...
MIPI_ATTR_S MIPI_2LANE_SENSOR_OV5647_60FPS_480P_10BIT_LINEAR_ATTR =
{
    .mipi_host_cfg = {
        .lane = 2, // 硬件上sensor用了几个mipi数据lane，f37用的1 lane, os8a10 4K的用了4 lane
        .datatype = 0x2b, // sensor的输出数据类型，请参考 《X3J3平台AIOT媒体系统接口手册.pdf》第3.5.36节 DATA TYPE
        ...
        .mipiclk = 1000, // sensor 输出总的 mipibit rate, 单位Mbits/秒
        .fps = 30, // sensor 输出实际帧率, 和 MIPI_SENSOR_INFO_S 是什么关系？
        .width = 640, // sensor实际输出的分辨率，一般根据sensor供应商的配置参数配置就行
        .height = 480,
        .linelenth = 1852, // sensor 输出带 blanking 的总行长， 一般根据sensor供应商的配置参数配置就行
        .framelenth = 504, // sensor 输出带 blanking的总行数
        .settle = 30, // sensor 输出实际Ttx-zero + Ttx-prepare时间（ clk 为单位）, 一般0-100之间尝试
		.channel_num = 1, // 使用虚通道的个数, linear 用一个，dol2 用 2 个， dol3 用3个
		.channel_sel = {0} // 保存每个虚通道的值，在调用HB_VIN_SetDevVCNumber接口时用第一个，调用HB_VIN_AddDevVCNumber用后面的2 3 4的配置
    },
...
VIN_DEV_ATTR_S DEV_ATTR_OV5647_60FPS_480P_LINEAR_BASE = {
    .stSize = { // 输入的数据
		.format = 0, // 像素格式， format 为 0 代表是raw, 根据 pixel_lenght 来表示究竟是 raw8, raw12 还是 raw16 。
		.width = 640, // 数据宽
		.height = 480, // 数据高
    ...
    .outDdrAttr = { // sif(dev) 的输出到 ddr 配置
        .stride = 800, // 硬件 stride 跟格式匹配，通过行像素根据raw数据bit位数计算得来（而且结果要32对齐），8bit：x1, 10bit: x1.25 12bit: x1.5,例F37 raw10，1920 x 1.25 = 2400
        .buffer_num = 8, // dev 输出到 ddr 的 buf 个数，最少可设置为6
        //.frameDepth = 1, // 默认不配置最多 get 的帧数 , buffer_num 是总 buff数量，建议 frameDepth 值最大是ddrOutBufNum – 4
    },
    .outIspAttr = { // sif 到 isp 一些属性设置
...
VIN_PIPE_ATTR_S PIPE_ATTR_OV5647_60FPS_480P_LINEAR_BASE = {
	.ddrOutBufNum = 6, // ISP->GDC ddr的buf数量，最少可以设置成 2
	.snsMode = SENSOR_NORMAL_MODE, // sensor 工作模式，要和MIPI_SENSOR_INFO_S配置的sensor_mode一致
	.stSize = { // 数据格式同 VIN_DEV_SIZE_S
		.format = 0,
...
VIN_DIS_ATTR_S DIS_ATTR_OV5647_60FPS_480P_LINEAR_BASE = {
	.picSize = { // 输入数据宽高
		.pic_w = 639, // 需要设置比接入尺寸 -1 的 size, 如果 ISP 输出1920 , 则这里设置 1919
		.pic_h = 479, // 需要设置比接入尺寸 -1 的 size
	},
	.disPath = { // 输出类型
		.rg_dis_enable = 0, // 是否开启
		.rg_dis_path_sel = 1, // 决定进dis的数据来源，0代表ISP输出给dis，1代表LDC输出给dis
...
VIN_LDC_ATTR_S LDC_ATTR_OV5647_60FPS_480P_LINEAR_BASE = {
  .ldcEnable = 0, // LDC 是否使能
  .ldcPath = { // 输出类型
	 .rg_y_only = 0, // ???
	 .rg_uv_mode = 0, // ???
  ...
  .cStartAddr = 786432, // Iram 使用地址的内存地址
  .picSize = { // 输入的尺寸
	 .pic_w = 639, // 需要设置比接入尺寸 -1 的 size, 如果 ISP 输出 1920 ,则这里设置 1919
	 .pic_h = 479,
  },
  .lineBuf = 99, // 值设置 99, 为什么？
  .xParam = { // 参数需要 tuning
